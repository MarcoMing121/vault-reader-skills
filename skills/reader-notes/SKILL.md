...
description: |
  ...
  - 对齐检查 (Alignment Check) - 新内容与原文冲突时，提示用户更新原文
...
> **核心变更**：不再无脑追加。每次添加内容前，检查新理解是否 supersede / 与原文矛盾，主动列出冲突让用户决定是否更新原文。
...
1. **直接矛盾** — 新说法 vs 原文说法，截然相反
2. **Supersede** — 新理解更本质，原文只是表层解释（如万向节锁案例）
3. **过时信息** — Q&A 中纠正过的错误认识，但原文还写着
4. **图不匹配** — Mermaid 架构图与最新理解不符

### 不触发检查的情况

- 纯粹追加新信息，不与原文冲突
- 补充阅读感想（"我觉得这个方法很巧妙"）
- 与其他论文的对比表格（除非对比结论与原文矛盾）
...
2. Agent 解释该部分
3. 用户确认："写到笔记里"
4. Agent **运行对齐检查**（见「对齐检查原则」），列出冲突项给用户确认
5. Agent 执行：
   - 找到该部分在笔记中的位置
